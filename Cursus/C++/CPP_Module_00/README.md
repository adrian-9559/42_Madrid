# CPP Module 00 — Fundamentos y entorno
...
Introducir el entorno de trabajo en C++: compiladores, flags de compilación, estructura básica de un proyecto y pequeñas utilidades para comenzar a programar en C++ moderno.

Temas principales

- Compilación con `clang++`/`g++`.
- Flags comunes: `-std=c++17`, `-Wall`, `-Wextra`, `-Werror`.
- Estructura de un proyecto simple (`src/`, `include/`, `Makefile`).
- Creación de pequeños programas de consola y ejercicios de práctica.

Ejercicios típicos

- «Hello, C++» con argumentos de línea de comandos.
- Programas para practicar entrada/salida y manejo de errores.

Cómo ejecutar

Si el proyecto incluye `Makefile`:
...
- Usa sanitizers (`-fsanitize=address,undefined`) durante las pruebas.
- Añade `-g` para depuración con `lldb` o `gdb`.
